...
func NewSignalFxClient(opts MetricsProviderOpts) (MetricsProviderClient, error) {
	if opts.Name != SignalFxClientName {
		return nil, fmt.Errorf("metric provider name should be %v, found %v", SignalFxClientName, opts.Name)
	}
...
func (s signalFxClient) FetchHostMetrics(host string, window *metricstype.Window) ([]metricstype.Metric, error) {
	...
	for _, metric := range []string{cpuUtilizationMetric, memoryUtilizationMetric} {
		...
		var fetchedMetric metricstype.Metric
...
func (s signalFxClient) FetchAllHostsMetrics(window *metricstype.Window) (map[string][]metricstype.Metric, error) {
...
func (s signalFxClient) Healthy() (int, error) {
...
func addMetadata(metric *metricstype.Metric, metricType string) {
	metric.Operator = metricstype.Average
	if metricType == cpuUtilizationMetric {
		metric.Name = cpuUtilizationMetric
		metric.Type = metricstype.CPU
	} else {
		metric.Name = memoryUtilizationMetric
		metric.Type = metricstype.Memory
	}
}

func (s signalFxClient) buildMetricURL(hostFilter string, clusterFilter string, metric string, window *metricstype.Window) (uri *url.URL, err error) {
...
*/
func getMetricsFromPayloads(metricData interface{}, metadata interface{}) (map[string]metricstype.Metric, error) {
	keyHostMap := make(map[string]string)
	hostMetricMap := make(map[string]metricstype.Metric)
	...
	for key, metric := range keyMetricMap {
		...
		fetchedMetric := metricstype.Metric{Value: sum / count}
